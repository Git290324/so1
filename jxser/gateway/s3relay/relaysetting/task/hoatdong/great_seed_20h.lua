...
greatseed_configtab = {
	...
	-- {336,3,100,"\\settings\\maps\\great_night\\phonglangdo.txt","Phong L¨ng §é"},			--·çÁê¶É
	...
	-- {167,1,25,"\\settings\\maps\\great_night\\diemthuongson.txt","§iÓm Th­¬ng S¬n"},			-- ³¤°×É½ÄÏ
	-- {200,1,25,"\\settings\\maps\\great_night\\coduongdong.txt","Cæ D­¬ng §éng"},	-- ³¤°×É½ÄÏ
	-- {92,1,25,"\\settings\\maps\\great_night\\thuccuongson.txt","Thôc C­¬ng S¬n"},				-- ³¤°×É½ÄÏ

	--{341, 4, 1, "\\settings\\maps\\great_night\\macbacthaonguyen.txt","M¹c B¾c Th¶o Nguyªn"},		-- Ä®±±²ÝÔ­
	-- {333, 4, 1, "\\settings\\maps\\great_night\\hoason.txt","Hoa S¬n Ph¸i"},			-- »ªÉ½ÅÉ
	-- {319, 4, 1, "\\settings\\maps\\great_night\\lamduquan.txt","L©m Du Quan"},			-- ÁÙÓå¹Ø
	{37, 2, 100, "\\settings\\maps\\great_night\\bienkinh.txt","bien kinh"},			-- Ë«Áú¶´
	-- {959, 4, 3, "\\settings\\maps\\great_night\\chienlongdong.txt","ChiÕn Long §éng"},			-- Ë«Áú¶´
	-- {181, 4, 1, "\\settings\\maps\\great_night\\luongthuydong.txt","L­ìng Thñy §éng"},			-- Á½Ë®¶´
};
...
function TaskContent()
	...
	if righttime_add() ~= 1 then--Èç¹ûÊ±¼ä²»¶Ô
		...
		gb_SetTask("QUA HUY HOANG", 22, 0);	--12ºÅ±äÁ¿ÓÃÀ´´æ´¢NPCµÄÐòºÅ
		return
	end;
	...
	if nNowTime >= 2200 and nNowTime < 2205 then	--	
...
function righttime_add()
	...
	if (nTime >= 2200 and nTime < 2230)  then
		return 1;
	end;
	return 0;
end

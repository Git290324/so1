...
function want_playboat_caocap_todoi()
	...
	if (GetTeamSize() < 2) then
		Say("Xin lçi! §éi tham gia 'NhiÖm vô Th¸ch thøc thêi gian' cÇn ph¶i cã Ýt nhÊt <color=red>2<color> ng­êi!",0)
		return
	end
	if(GetLevel() < 50) then
	...
	local strlevel ="";
	if (blevel == 0) then
		strlevel = "V­ît ¶i (s¬ cÊp -  tæ ®éi)"
	else
		strlevel = "V­ît ¶i (cao cÊp -  tæ ®éi)"
	end
	...
	if (startboat == 1) then
		if (freeboat == 0) then
			...
			Say(""..strlevel.."Yªu cÇu : <enter>1 v¹n l­îng<enter>1 s¸t thñ gi¶n",2, "Ph¶i! Ta muèn dÉn d¾t ®éi cña ta tham gia./dragon_join_caocap_todoi", "Kh«ng cÇn/onCancel")
			return
		end
	else
...
function dragon_join_caocap_todoi()
	...
	for i = 1, GetTeamSize() do 
		...
		if (havesword < 1 ) then
			Say("NhiÕp ThÝ ThÇn: H·y chuÈn bÞ 1 s¸t thñ gi¶n råi quay l¹i gÆp ta",0)
			Msg2Team("Trong tæ ®éi cña ng­¬i cã ng­êi kh«ng cã S¸t Thñ Gi¶n , xin  kiÓm tra l¹i råi h·y b¸o danh tham gia !")
			return
		end
	end

...
function tbGmRole:GMC_Menu()
		...
		local tbOpt =
	{
		...
		{"Qu¶n l? Ng­êi Ch¬i", QLG}, 
		{"D?CH chuyÓn tøc thêi", g_AskClientStringEx, {"53,200,201", 0, 256, "Map,X,Y", {self.DenDD, {self}} }}, 
		--{"§?n b¶n ®å c«ng thµnh Chi?n", gopos_sevenctc},
		{"Th«ng b¸o quan träng dµnh cho GM", thongbaoquantrong},
		{"Kick All nh©n vËt ®ang Online",kickroll},
		{"T×m V? tr? nh©n vËt", g_AskClientStringEx, {GetName(), 0, 300, "Tªn nh©n vËt", {self.FindRole, {self}} }}, 
		...
		{"Khëi §éng T?nh N¨ng Games",StartGame},
		--{"K?t Thóc Hç Trî"}
	}
...
elseif CheckAccountMod() then
	...
	CreateNewSayEx(szTitle, tbOpt)
		...
		Msg2Player("ChØ c? <color=orange>GM HoÆc Mod<color> míi sö dông ®­îc vËt phÈm nµy. !");
		return 0
end
...
function gopos_sevenctc()
	Say("Ng­¬i muèn ®i Chi?n tr­êng nµo cña ThÊt Thµnh §¹i Chi?n?", 8,
		"Chi?n tr­êng Thµnh §«/#goto_ctc(1)",
		"Chi?n tr­êng BiÖn Kinh/#goto_ctc(2)",
		"Chi?n tr­êng §¹i L?/#goto_ctc(3)",
		"Chi?n tr­êng Ph­îng T­êng/#goto_ctc(4)",
		"Chi?n tr­êng L©m An/#goto_ctc(5)",
		"Chi?n tr­êng T­¬ng D­¬ng/#goto_ctc(6)",
		"Chi?n tr­êng D­¬ng Ch©u/#goto_ctc(7)",
...
----------------------copy toµn bé néi dung t? ®©y ®?n h?t vµo cuèi cïng lÖnh bµi cñ b¹n-----------------------------------
function StartGame()

Say("H·y Lùa Chän Chøc N¨ng Khëi §éng",20,
	 --"Gäi Boss Ki?m Tiªn L? B¹ch/Gm_Call_Boss_KiemTienLyBach",
	 --"Gäi Boss §¹i Hoµng Kim /Gm_Call_Big_Boss",
	 --"Gäi Boss §¹i TiÓu Hoµng Kim /Gm_Call_Small_Boss",
	-- "Gäi Boss §éc C« Thiªn Phong 19h45 /Gm_Call_DocCoTP",
	"GM - Më Ho¹t §éng Tèng Kim/KDTongKim",
      "K?t Thóc §èi Tho¹i/No")
end

function KDTongKim()
	local tbSay = {"<dec>Mêi GM tr·i nghiÖm chøc n¨ng trong game"};
		tinsert(tbSay, "GM - B¾t ®Çu b¸o danh/BaoDanhTK")
		-- tinsert(tbSay, "GM - B¾t ®Çu chi?n ®Êu/BatDauChienDau")
		-- tinsert(tbSay, "GM - K?t thóc chi?n ®Êu/KetThucChienDau")
		tinsert(tbSay, "§?ng./no");
	CreateTaskSay(tbSay);
end
...
RemoteExc("\\script\\frank\\frank_callhoatdong.lua", "Gm_Call_Battles", {nNum})
end
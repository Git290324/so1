...
function main()
	...
	tinsert(bilTbOp, "Mua B¾c §Èu Tr­êng Sinh ThuËt - C¬ Së Thiªn (1000 V¹n)/chuyensinh")
	tinsert(bilTbOp, "Mua B¾c §Èu Tr­êng Sinh ThuËt - ®¹i thõa t©m ph¸p (2000 V¹n)/chuyensinhthu4")
	tinsert(bilTbOp, "Mua TÝch LÞch ®¬n x999 (1000 V¹n)/trungsinhlan4")
	--tinsert(bilTbOp, "Mua Thæ ®Þa phï (50 V¹n)/MuaThodia")
	--tinsert(bilTbOp, "Mua mua tói m¸u tèng kim (20 V¹n)/Muamau")
...
function MuaThanHanh()
	...
	Pay(500000)
	local tbItem = {szName="ThÇn hµnh phï", tbProp={6,1,1266,1,0,0}, nBindState=-2,nExpiredTime = 43830}
	tbAwardTemplet:GiveAwardByList(tbItem, "ThÇn hµnh phï", 1)

end
function chuyensinh()
	if (GetCash() <= 10000000) then
		Talk(1, "", "Ph¶i ca ®ñ 1000 v¹n míi ca thÓ mua.")
		return
	end
	Pay(10000000)
	local tbItem = {szName="B¾c §Èu Tr­êng Sinh ThuËt - C¬ Së Thiªn", tbProp={6,1,1390,1,0,0}, nBindState=0,nExpiredTime = 0}
	tbAwardTemplet:GiveAwardByList(tbItem, "B¾c §Èu Tr­êng Sinh ThuËt - C¬ Së Thiªn", 1)

end
function chuyensinhthu4()
	if (GetCash() <= 20000000) then
		Talk(1, "", "Ph¶i ca ®ñ 2000 v¹n míi ca thÓ mua.")
		return
	end
	Pay(20000000)
	local tbItem = {szName="B¾c §Èu Tr­êng Sinh ThuËt - ®¹i thõa t©m ph¸p", tbProp={6,1,2974,1,0,0}, nBindState=0,nExpiredTime = 0}
	tbAwardTemplet:GiveAwardByList(tbItem, "B¾c §Èu Tr­êng Sinh ThuËt - ®¹i thõa t©m ph¸p", 1)

end
function trungsinhlan4()
	if (GetCash() <= 10000000) then
		Talk(1, "", "Ph¶i ca ®ñ 1000 v¹n míi ca thÓ mua.")
		return
	end
	if (CalcFreeItemCellCount() < 20) then
		CreateTaskSay({"<dec><npc>".."Hµnh trang kh«ng ®ñ 20 «, xin mêi h·y thu dän hµnh lý.", "KÕt thóc ®èi tho¹i/OnCancel"});
		return
	end
	Pay(10000000)
	local tbItem = {szName="TÝch LÞch ®¬n x999", tbProp={6,1,2973,999,0,0}, nBindState=0,nExpiredTime = 0}
	tbAwardTemplet:GiveAwardByList(tbItem, "B¾c §Èu Tr­êng Sinh ThuËt - C¬ Së Thiªn", 999)
...
function MuaThodia()
	...
	Pay(500000)
	local tbItem = {szName="Thæ ®Þa phï", tbProp={6,1,438,1,0,0}, nBindState=-2,nExpiredTime = 43830}
	tbAwardTemplet:GiveAwardByList(tbItem, "Thæ ®Þa phï", 1)
...
function Muamau()
	...
	Pay(200000)
	local tbItem = {szName="mua tói m¸u tèng kim", tbProp={6,1,2527,1,0,0}, nBindState=-2,nExpiredTime = 43830}
	tbAwardTemplet:GiveAwardByList(tbItem, "mua tói m¸u tèng kim", 1)

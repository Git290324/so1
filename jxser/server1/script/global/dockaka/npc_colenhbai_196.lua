...
function main()
    dofile("script/global/dockaka/npc_colenhbai_196.lua");
...
function DealWithIsCaption()
	...
	for i = 1, nTeamSize do
		...
		local nHavePass = DynamicExecuteByPlayer(nPlayerIndex, "", "CalcItemCount", -1, 6, 1, 196, -1)
...
function SingleEnterSure()
	...
	if not VnCheckPass(PlayerIndex) or not VnConsumePass(PlayerIndex) then
		Talk(1, "", "ChØ cÇn giao cho ta 1 c¸i <color=red>mËt ®å thÇn bÝ x100<color> ta sÏ chØ cho ng­¬i lèi vµo.")
		return
	end
...
function TeamEnterSure()
	...
	for i = 1, nTeamSize do 
		...
		if VnConsumePass(nPlayerIndex) then
			...
			local nPosX = tonumber(TabFile_GetCell(szConfigPath, nRowId, "TRAPX"))
			local nPosY = tonumber(TabFile_GetCell(szConfigPath, nRowId, "TRAPY"))
			DynamicExecuteByPlayer(nPlayerIndex, "", "NewWorld", 969, 1580, 3191)			
		end		
	end
end
...
function VnCheckPass(nPlayerIdx)
	...
	if CalcItemCount(-1, 6, 1, 196, -1) < 100 then		
...
function VnConsumePass(nPlayerIdx)
	local nOldPlayer = PlayerIndex
	PlayerIndex = nPlayerIdx
	if ConsumeItem(-1, 100, 6, 1, 196, -1) ~= 1 then
		PlayerIndex = nOldPlayer
		return nil
	end
	PlayerIndex = nOldPlayer
...
function VnCheckTeamCondition(bShowDialog1, bShowDialog2)
	...
	for i = 1, nTeamSize do		
		...
		if not VnCheckPass(PlayerIndex) then
			tinsert(tbDialog, getn(tbDialog) + 1, format("<color=red>%s<color>: %s", GetName(), "Kh«ng cã mËt ®å thÇn bÝ x100"))
		end	
	end
	PlayerIndex = nOldPlayer

...
function get_ingot()
	...
	if (nMoney <= 0) then					-- Ã»ÓĞÀ©Õ¹µãÊı
		...
		local nCurVar = nMoney *10
		Say("<#> Kh¸ch quan ®ang göi ë ®©y tæng céng <color=green>"..nCurVar.."<color> TiÒn ®ång, muèn rót chø?", 4, "Ta muèn rót 10 TiÒn ®ång/get_ingot10","Ta muèn rót 50 TiÒn ®ång/get_ingot50","Ta muèn rót 100 TiÒn ®ång/get_ingot100", "Kh«ng rót/no") 
	end
end
--------------- rut 10 xu
...
function get_ingot50()
	...
	if (GetExtPoint(1) < 5) then
		Talk(1,"","Kh«ng ®ñ 50 tiÒn ®ång") 
		return
	end
	Say("Kh¸ch quan cã ch¾c muèn rót TiÒn ®ång chø? (NÕu ®ång ı, xin ®õng tho¸t khái trß ch¬i!) ", 2, "Ta ®ång ı/get_ingot50ok", "Kh«ng rót/no"); 
end
...
function get_ingot100()
	...
	if (GetExtPoint(1) < 10) then
		Talk(1,"","Kh«ng ®ñ 100 tiÒn ®ång") 
		return
	end
	Say("Kh¸ch quan cã ch¾c muèn rót TiÒn ®ång chø? (NÕu ®ång ı, xin ®õng tho¸t khái trß ch¬i!) ", 2, "Ta ®ång ı/get_ingot100ok", "Kh«ng rót/no"); 
end

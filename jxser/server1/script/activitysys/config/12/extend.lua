...
function pActivity:GiveTask()
	...
	for k,v in self.tbWinners do
		if k == szName then
			return Talk(2, "", "PhÇn th­ëng lÇn tr­íc ng­¬i vÉn ch­a nhËn.","ta nhan thuong/teo_thuongnhantieu")
		end
	end
...
function pActivity:TaskFinish(nTaskId, nNpcIndex)
	...
	local  nPlayerIndex = SearchPlayer(szName)
	if nPlayerIndex > 0 then
		CallPlayerFunction(nPlayerIndex, Msg2Player, "<enter>NhiÖm vô hoµn thµnh<enter>nhËn  th­ëng hép quµ x150")
	end
	
	for teo_thuongnhantieu = 1,150 do
	  AddItem(6,3399);
	end
